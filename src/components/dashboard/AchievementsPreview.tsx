            ...
            <div className="grid grid-cols-6 gap-2 flex-1 overflow-y-auto scrollbar-thin auto-rows-fr">
                {allBadges.map((badge) => {
                    ...
                    return (
                        <div
                            key={badge.id}
                            className={`relative flex flex-col items-center justify-center p-2 rounded-xl border text-center transition-all duration-300 ${earned ? `${badge.bgClass} ${badge.glowClass}` : "bg-muted/10 border-border/30"
                                }`}
                            title={`${badge.title} — ${badge.description}`}
                        >
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-1.5 ${earned ? badge.bgClass : "bg-muted/20"
                                }`}>
                                <badge.icon size={20} className={earned ? badge.colorClass : "text-muted-foreground/40"} />
                            </div>
                            <span className={`text-[9px] font-mono leading-tight text-center w-full ${earned ? "text-foreground font-bold" : "text-muted-foreground/40"
                                }`}>
                                {badge.title}
                            </span>
                            {earned && (
