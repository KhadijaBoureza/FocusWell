...
function calculateDailyTaskTrend(data: WeeklyDataItem[]) {
  if (data.length < 2) return 0;

  const yesterdayTasks = data[data.length - 2].tasks;
  const todayTasks = data[data.length - 1].tasks;
...
function calculateDailyFocusTrend(data: WeeklyDataItem[]) {
  if (data.length < 2) return 0;

  const yesterdayFocus = data[data.length - 2].focus;
  const todayFocus = data[data.length - 1].focus;

  if (yesterdayFocus === 0) {
    return todayFocus > 0 ? 100 : 0;
  }

  return Math.round(((todayFocus - yesterdayFocus) / yesterdayFocus) * 100);
}
...
function WeeklyAnalytics({ tasks, refreshKey }: WeeklyAnalyticsProps) {
  ...
  const taskTrend = calculateDailyTaskTrend(weeklyData);
  const focusTrend = calculateDailyFocusTrend(weeklyData);
    ...
    <div className="glass-card neon-border-blue p-6">
          ...
          <div>
            <div
              className={`flex items-center justify-end gap-1 text-xs ${
                focusTrend >= 0 ? "text-green-500" : "text-red-500"
              }`}
            >
              <TrendingUp size={14} />
              {focusTrend >= 0 ? "+" : ""}
      ...
      <div className="mb-5 grid grid-cols-3 gap-3">
        <div className="rounded-lg bg-muted/40 p-3 text-center">
          <p className="text-xl font-bold">{formatFocus(totalFocus)}</p>
        ...
        <div className="rounded-lg bg-muted/40 p-3 text-center">
          <p className="text-xl font-bold">
            {formatFocus(Math.ceil(totalFocus / 7))}
          </p>
